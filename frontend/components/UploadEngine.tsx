  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setResult(resultJson as UploadResult);
      }
    } catch (error) {
  ...
  return (
    <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-xl shadow-slate-950/20">
      <div className="mb-8">
        ...
        <p className="mt-3 text-slate-300">Upload a document and let the platform analyze invoices, receipts, and payment details automatically.</p>
      </div>
      ...
      {result ? (
        <div className="mt-10 rounded-3xl border border-slate-800 bg-slate-950/90 p-6">
          <div className="mb-6 flex items-center justify-between gap-4">
            ...
            <span className="rounded-full bg-slate-800 px-4 py-2 text-sm text-slate-300">Match {Math.round(result.parsed_invoice.confidence * 100)}%</span>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5">
              ...
              <p className="mt-2 text-lg font-semibold text-white">${result.parsed_invoice.total.toFixed(2)}</p>
            </div>
            <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5">
